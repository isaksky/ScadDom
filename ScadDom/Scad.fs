...
module internal OutputHelpers =
    ...
    let internal renderArgs (tw: TextWriter) (args: FnArg list) =
    ...
    let internal renderOutput (tw: TextWriter) (output: OutputTree) =
        let pendingModifiers = Queue<string>()
        renderOutput' tw output 0 pendingModifiers

type Document() =
    member val DocumentPrelude = "" with get, set
    member val DomNodes = ResizeArray<ScadDomNode>() with get  

type Scad private() =
    static let renderDoc (sw: TextWriter) (doc: Document) =
        let mutable firstLine = true
        
        if doc.DocumentPrelude.Length > 0 then
            sw.WriteLine doc.DocumentPrelude
            firstLine <- false
            
        for dom in doc.DomNodes do
            sw.Write("\n")
            let outputTree = domToOutputTree dom
            renderOutput sw outputTree

    static member Render (dom: ScadDomNode) : string =
        use sw = new StringWriter()
        let outputTree = domToOutputTree dom
        renderOutput sw outputTree
        sw.ToString()
        
    static member Render (doc: Document) : string =
        use sw = new StringWriter()        
        renderDoc sw doc
        sw.ToString()        

    static member Render(dom: ScadDomNode, sw: TextWriter, leaveOpen: bool) : unit =
        try
            let outputTree = domToOutputTree dom
            renderOutput sw outputTree
        finally
    ...
    static member Render(doc: Document, sw: TextWriter, leaveOpen: bool) : unit =
        try
            renderDoc sw doc
        finally
            if not leaveOpen then
    ...
    static member Render(dom: ScadDomNode, path: string) : unit =
        use fs = File.Open(path, FileMode.Create)
        use sw = new StreamWriter(fs)
        let outputTree = domToOutputTree dom
        renderOutput sw outputTree
        
    static member Render(doc: Document, path: string) : unit =
        use fs = File.Open(path, FileMode.Create)
        use sw = new StreamWriter(fs)        
        renderDoc sw doc        
